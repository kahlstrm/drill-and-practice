...
const getOptionsById = async (question_id) => {
  const res = await executeQuery(
    "SELECT o.*,count(qa.id) FROM question_answer_options AS o LEFT JOIN question_answers as qa ON o.id=qa.question_answer_option_id WHERE o.question_id=$question_id GROUP BY (o.id);",
    { question_id }
  );
  return res.rows;
};
